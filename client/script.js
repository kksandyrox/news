	...
	scotchApp.controller('generalController', function($scope, $http, $route, $location) {
		...
		$http.get("http://" + $location.host() + ":8000/categoryNews/" + category_id)
	...
	scotchApp.controller('sourceNewsController', function($scope, $http, $route, $location) {
		...
		$http.get("http://" + $location.host() + ":8000/sources")
		.then(function (response) {
			$scope.sources = response.data;
		})

		$http.get("http://" + $location.host() + ":8000/sourceNews/" + source_id)
		.then(function (response) {
